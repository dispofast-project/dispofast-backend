...
public class OrderEmailComposer {
  ...
  public void sendOrderCreatedEmail(SalesOrder order, List<SalesOrderItemResponseDTO> items) {
    try {
      ...
      String body =
          buildEmailHtml(
              order, items, subtotal, tax, discountAmt, additionalDiscountAmt, retefuente, freight,
              total);
  ...
  private String buildEmailHtml(
    ...
    String asesorName =
        order.getAsesor() != null ? esc(order.getAsesor().getFullName()) : "-";
    ...
    for (int i = 0; i < items.size(); i++) {
      ...
      itemRows
          .append("<tr style=\"background-color:").append(rowBg).append(";\">")
          .append("<td style=\"padding:7px 10px;border-bottom:1px solid #e8e8e8;font-size:12px;color:#555;\">")
          .append(esc(item.getProductReference())).append("</td>")
          .append("<td style=\"padding:7px 10px;border-bottom:1px solid #e8e8e8;font-size:12px;\">")
          .append(esc(item.getProductName())).append("</td>")
          .append("<td style=\"padding:7px 10px;border-bottom:1px solid #e8e8e8;font-size:12px;text-align:center;\">")
          .append(item.getQuantity().stripTrailingZeros().toPlainString()).append("</td>")
          .append("<td style=\"padding:7px 10px;border-bottom:1px solid #e8e8e8;font-size:12px;text-align:right;\">")
          .append(fmt(item.getLineTotal())).append("</td>")
          .append("<td style=\"padding:7px 10px;border-bottom:1px solid #e8e8e8;font-size:12px;text-align:right;\">")
          .append(fmt(ivaAmt)).append("</td>")
          .append("<td style=\"padding:7px 10px;border-bottom:1px solid #e8e8e8;font-size:12px;text-align:right;font-weight:bold;\">")
          .append(fmt(rowTotal)).append("</td>")
          .append("</tr>");
    }
    ...
    return "<!DOCTYPE html><html lang=\"es\"><head><meta charset=\"UTF-8\"/></head>"
        ...
        // ── HEADER ────────────────────────────────────────────────
        + "<div style=\"background-color:" + PRIMARY + ";padding:24px 30px;text-align:center;\">"
        ...
        + esc(order.getOrderNumber()) + "</p>"
        + "</div>"

        // ── CLIENTE ───────────────────────────────────────────────
        + "<div style=\"padding:22px 30px;background-color:#f7f9fc;border-bottom:1px solid #e1e6ed;\">"
        + "<h2 style=\"color:" + PRIMARY + ";margin:0 0 14px;font-size:13px;text-transform:uppercase;letter-spacing:0.5px;\">Informaci&oacute;n del Cliente</h2>"
        ...
        + "<h2 style=\"color:" + PRIMARY + ";margin:0 0 14px;font-size:13px;text-transform:uppercase;letter-spacing:0.5px;\">Datos de la Orden</h2>"
        + "<table style=\"width:100%;border-collapse:collapse;font-size:13px;\">"
        ...
        + "<div style=\"padding:22px 30px;border-bottom:1px solid #e1e6ed;\">"
        + "<h2 style=\"color:" + PRIMARY + ";margin:0 0 14px;font-size:13px;text-transform:uppercase;letter-spacing:0.5px;\">Listado de Productos</h2>"
        + "<table style=\"width:100%;border-collapse:collapse;font-size:12px;\">"
        + "<thead><tr style=\"background-color:" + PRIMARY + ";\">"
        ...
        // ── RESUMEN FINANCIERO ────────────────────────────────────
        + "<div style=\"padding:22px 30px;border-bottom:1px solid #e1e6ed;\">"
        + "<h2 style=\"color:" + PRIMARY + ";margin:0 0 14px;font-size:13px;text-transform:uppercase;letter-spacing:0.5px;\">Resumen Financiero</h2>"
        + "<table style=\"width:100%;border-collapse:collapse;font-size:13px;\">"
  ...
  private static String row(String label, String value) {
    return "<tr>"
        + "<td style=\"padding:5px 0;color:#666;width:40%;\">" + label + "</td>"
        + "<td style=\"padding:5px 0;color:#222;\">" + value + "</td>"
        + "</tr>";
  }

  private static String sumRow(String label, String value, boolean highlight) {
    String style = highlight
        ? "padding:10px 0;color:" + PRIMARY + ";font-weight:bold;font-size:14px;border-top:2px solid " + PRIMARY + ";"
        : "padding:5px 0;color:#666;";
    String valueStyle = highlight
        ? "padding:10px 0;text-align:right;color:" + PRIMARY + ";font-weight:bold;font-size:14px;border-top:2px solid " + PRIMARY + ";"
